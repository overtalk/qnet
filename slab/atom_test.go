...
func Test_AtomPool_AllocAndFree(t *testing.T) {
	...
	for i := 0; i < len(pool.pages); i++ {
		temp := make([][]byte, len(pool.pages[i].chunks))

		for j := 0; j < len(temp); j++ {
			mem := pool.Alloc(pool.pages[i].size)
			utest.EqualNow(t, cap(mem), pool.pages[i].size)
			temp[j] = mem
		}
		utest.Assert(t, pool.pages[i].head == 0)
...
func Test_AtomPool_AllocSlow(t *testing.T) {
	pool := NewAtomPool(128, 1024, 2, 1024)
	mem := pool.pages[len(pool.pages)-1].Pop()
	utest.EqualNow(t, cap(mem), 1024)
	utest.Assert(t, pool.pages[len(pool.pages)-1].head == 0)
